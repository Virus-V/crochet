...
nanopi-neo4_check_uboot ( ) {
	uboot_port_test ${UBOOT_DIR} ${UBOOT_BIN}
}
strategy_add $PHASE_CHECK nanopi-neo4_check_uboot

#
# NanoPi-NEO4 uses EFI, so the first partition will be a FAT partition.
#
nanopi-neo4_partition_image ( ) {
...
strategy_add $PHASE_PARTITION_LWW nanopi-neo4_partition_image

nanopi-neo4_populate_boot_partition ( ) {
	mkdir -p efi/boot
	echo bootaa64 > startup.nsh
}
strategy_add $PHASE_BOOT_INSTALL nanopi-neo4_populate_boot_partition
...
# NanoPi-NEO4 puts the kernel on the FreeBSD UFS partition.
...
fix_dtb_path () {
	...
	DTBFILE="${BOARD_FREEBSD_MOUNTPOINT}/boot/dtb/rockchip/rk3399-nanopi-neo4.dtb"
	if [ -f ${DTBFILE} ] ; then
