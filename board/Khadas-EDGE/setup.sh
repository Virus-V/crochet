...
TARGET=aarch64
TARGET_ARCH=aarch64
UBOOT_DIR="u-boot-khadas-edge"
UBOOT_PATH="/usr/local/share/u-boot/${UBOOT_DIR}"
UBOOT_BIN="u-boot.itb"
IMAGE_SIZE=$((1000 * 1000 * 1000))

khadas_edge_check_uboot ( ) {
	uboot_port_test ${UBOOT_DIR} ${UBOOT_BIN}
}
strategy_add $PHASE_CHECK khadas_edge_check_uboot

#
# Khadas EDGE uses EFI, so the first partition will be a FAT partition.
#
khadas_edge_partition_image ( ) {
...
strategy_add $PHASE_PARTITION_LWW khadas_edge_partition_image

khadas_edge_populate_boot_partition ( ) {
	mkdir -p efi/boot
	echo bootaa64 > startup.nsh
}
strategy_add $PHASE_BOOT_INSTALL khadas_edge_populate_boot_partition
...
fix_dtb_path () {
	...
	DTBFILE="${BOARD_FREEBSD_MOUNTPOINT}/boot/dtb/rockchip/rk3399-khadas-edge.dtb"
	if [ -f ${DTBFILE} ] ; then
